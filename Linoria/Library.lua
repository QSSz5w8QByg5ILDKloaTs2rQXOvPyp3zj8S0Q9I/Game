...
do
    ...
    function BaseGroupboxFuncs:AddSlider(Idx, Info)
        ...
        if not Info.Compact and not Info.SubSlider then
        ...
        function Slider:AddSlider(SubIdx, SubInfo)
            assert(SubInfo.Default, 'AddSlider: Missing default value.');
            assert(SubInfo.Text, 'AddSlider: Missing slider text.');
            assert(SubInfo.Min, 'AddSlider: Missing minimum value.');
            assert(SubInfo.Max, 'AddSlider: Missing maximum value.');
            assert(SubInfo.Rounding, 'AddSlider: Missing rounding value.');

            self.Outer.Size = UDim2.new(0.5, -2, 0, self.Outer.AbsoluteSize.Y)

            SubInfo.SubSlider = true

            local SubSlider = BaseGroupboxFuncs.AddSlider(Groupbox, SubIdx, SubInfo)
            SubSlider.Outer.Position = UDim2.new(1, 3, 0, 0)
            SubSlider.Outer.Size = UDim2.new(1, -3, 0, self.Outer.AbsoluteSize.Y)
            SubSlider.Outer.Parent = self.Outer
